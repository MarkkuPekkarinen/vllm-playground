...
This release fixes four bugs and adds an embedded Tutorials page.
...
**Fix:** `find_claude_command()` now checks well-known per-user install paths (`~/.local/bin/claude`, `~/.npm-global/bin/claude`) as fallbacks before falling back to the npm global bin lookup.

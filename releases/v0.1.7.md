...
**Fix:** `stop_server()` now clears all metrics data (`MetricStore.latest`, `MetricStore.history`, `latest_vllm_metrics`, `metrics_history`) when the server is stopped, so the observability dashboard starts fresh for the next session.

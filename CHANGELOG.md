...
- 🐛 **Stale observability data after mode switch** - Metrics are now cleared when the server stops, so the observability dashboard no longer shows data from a previous session after switching modes.
